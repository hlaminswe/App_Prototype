...
/**
 * Created by Min Swe on 11/25/2017.
 */
...
/**
 * It represents a set of indicator dots which when attached
...
public class PinIndicator extends LinearLayout {
    ...
    public PinIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PinLockView);

        try {
            mDotDiameter = (int) typedArray.getDimension(R.styleable.PinLockView_dotDiameter, ResourceUtils.getDimensionInPx(getContext(), R.dimen.rec_dot_diameter));
            mDotSpacing = (int) typedArray.getDimension(R.styleable.PinLockView_dotSpacing, ResourceUtils.getDimensionInPx(getContext(), R.dimen.rec_dot_spacing));
            mFillDrawable = R.drawable.rect_filled_with_circle;
            mEmptyDrawable = R.drawable.rect_empty;
